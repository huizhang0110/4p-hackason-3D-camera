...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    # inputs: [32768, 64, 3]       v.s.  [48048, 1, 3]
    # inputs_flat: [2097152, 3]
    # embeded: [2097152, 63]
    # viewdirs: [32768, 3]         v.s.  [1, 15, 3]
    # input_dirs: [32768, 64, 3]
    # input_dirs_flat: [2097152, 3]
    # embeded_dirs: [2097152, 27]
    # embedded: [2097152, 90]
    # outputs_flat: [2097152, 4]
    # outputs: [32768, 64, 4]
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    if raw_noise_std > 0.:
        ...
        if pytest:
            ...
            noise = torch.Tensor(noise).to(device=raw.device)

    alpha   = raw2alpha(raw[..., 3] + noise, dists.to(device=raw.device))  # [N_rays, N_samples]
...
def render_rays(ray_batch,
    ...
    viewdirs = ray_batch[:,-3:] if ray_batch.shape[-1] > 8 else None  # TODO: 这里表示什么意思？
    bounds = torch.reshape(ray_batch[..., 6:8], [-1, 1, 2]) 
    near, far = bounds[..., 0], bounds[..., 1] # [-1, 1]

    t_vals = torch.linspace(0., 1., steps=N_samples)                  # 线性采样N_samples个点 
    if not lindisp:                                                   # 在深度方向线性采样 
        z_vals = near * (1. - t_vals) + far * (t_vals) 
    else:
        z_vals = 1. / (1. / near * (1. - t_vals) + 1./ far * (t_vals))

    z_vals = z_vals.expand([N_rays, N_samples])                       # 拓展到整个batch 
    ...
    # rays_d:  [N_rays, 3]
    # rays_o:  [N_rays, 3]
    # repeat N_samples' rays_d & rays_o, 并且根据z_vals生成点集合  [N_rays, N_samples, 3]
    pts = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[..., :, None]

    # raw = run_network(pts)
    raw = network_query_fn(pts, viewdirs, network_fn)   # Call network计算得到alpha和rgb [N_rays, N_samples, 4]
    # 将模型的预测值alpha, rgb转换成语义上面有意义的值（图片）
    ## 具体步骤：
    ### 1) 论文中公式 2和3 
    ### 2) 
...
def train():
    ...
    @torch.no_grad()
    def nerf_eval_func(points, dirs):
        result = network_query_fn(points, dirs, network_fn)
        return result[..., :3], result[..., 3:]

    from nerfvis import nerfvis
    scene = nerfvis.Scene("doll-test")
    scene.set_nerf(nerf_eval_func, center=[0.0, 0.0, -1.5], radius=0.9, use_dirs=True)
