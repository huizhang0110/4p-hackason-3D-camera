...
void Draw::drawText(const char* textToDraw, int x, int y)
{
...
void Draw::ShowImagesSideBySide(const char * title, vector<IplImage> images, const char* text, int X_textOnWindow, int Y_textOnWindow)
{
    ...
    cv::Mat m1 = cv::cvarrToMat(DisplayImage);
    cv::imwrite("./test.jpg", m1);
